...
def main():
    ...
    print("*"*60)
    print(f"Evaluation toutes entités confondues ({len(entities)} entités gold / {nb_ocr_all_entities} entités OCR)")
    print("*"*60)
    eval_all_entities(entities, sentences)

    # Evaluation uniquement sur les PER
    type_ent = 'PER'
    ...
    # Evaluation uniquement sur les ORG
    type_ent = 'ORG'
    nb_gold_org = sum(1 for ent in entities if ent.gold_label == type_ent)
    nb_ocr_org = sum(1 for ent in entities if ent.ocr_label == type_ent)
    persons = [ent for ent in entities if ent.gold_label == type_ent or ent.ocr_label == type_ent]
    print("*"*60)
    print(f"Evaluation uniquement sur les organisations ({nb_gold_org} entités gold / {nb_ocr_org} entités OCR)")
    print("*"*60)
    eval_type_entities(persons, type_ent, sentences)

    # Evaluation uniquement sur les LOC
    type_ent = 'LOC'
    nb_gold_loc = sum(1 for ent in entities if ent.gold_label == type_ent)
    nb_ocr_loc = sum(1 for ent in entities if ent.ocr_label == type_ent)
    persons = [ent for ent in entities if ent.gold_label == type_ent or ent.ocr_label == type_ent]
    print("*"*60)
    print(f"Evaluation uniquement sur les localisations ({nb_gold_loc} entités gold / {nb_ocr_loc} entités OCR)")
    print("*"*60)
    eval_type_entities(persons, type_ent, sentences)
