...
def main():
    ...
    try:
        ...
        # Calcul du CER moyen
        df['cer'] = df.apply(lambda row: cer(row['Ground Truth'], row['Model Correction']), axis=1)
        mean_cer = df['cer'].mean()
        print(f"CER moyen : {mean_cer}")

        # Calcul du WER moyen
        df['wer'] = df.apply(lambda row: wer(row['Ground Truth'], row['Model Correction']), axis=1)
        mean_wer = df['wer'].mean()
        print(f"WER moyen : {mean_wer}")
