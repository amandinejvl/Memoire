...
def get_char_score(orig_word, candidate, conf_matrix, weight=0.1):
    """Attribue un score de probabilité basé sur la ressemblance visuelle des caractères"""
    score = 0

    if len(orig_word) == len(candidate):

        for orig_char, cand_char in zip(orig_word, candidate):

            try:
                score += conf_matrix.loc[orig_char, cand_char]
        ...
        common_chars = lcs(orig_word, candidate)
        score = len(common_chars)-abs(len(orig_word)-len(candidate))
...
def add_likely_words(orig_word, likely_words, conf_matrix):
    ...
    for i in range(len(likely_words)):
        ...
        if abs(len(orig_word)-len(likely_words[i].word)) <= round(len(orig_word)-0.7*len(orig_word)):
            ...
            if lcs > 0:
                ...
                # Calculer son score de similarité visuelle
                vis = get_char_score(orig_word, likely_words[i].word, conf_matrix)
...
def add_closest_words(sentence, orig_word, closest_words, conf_matrix):
    ...
    for i in range(len(closest_words)):
        ...
        if abs(len(orig_word)-len(closest_words[i])) <= 0.7*len(orig_word):
            ...
            if lcs > 0:
                ...
                vis = get_char_score(orig_word, closest_words[i], conf_matrix)
...
def find_most_freq_first(candidates):
    ...
    # Liste des scores
    scores = ['bert_score', 'distance', 'lcs', 'vis', 'final']
...
def find_best_candidate(sentence, orig_word, three_best_candidates):
    ...
    if three_best_candidates != []:
        ...
        if np.std(bert_scores) < 2:
            ...
            # Si il n'y a qu'un seul premier
            if len(premiers) == 1:
            ...
            # Sinon (si il y a plusieurs premiers)
            else:

                # On garde le candidat qui a le score visuel le plus élevé
                corr = sorted(premiers, key=lambda x: x.vis, reverse=True)[0].word
...
def compare_lexique_context(sentence: str, orig_word: str, closest_words: list, likely_words: list, conf_matrix):
    ...
    scores_likely_words = add_likely_words(orig_word, likely_words, conf_matrix)

    # Ajouter les mots proches à la liste des candidats
    scores_closest_words = add_closest_words(sentence, orig_word, closest_words, conf_matrix)
...
def correct_sentence(sent: str, ref_words:list, tokenizer, conf_matrix) -> str:
    """Corrige une phrase"""
            ...
            # Sinon (si le mot contient plusieurs tokens)
                ...
                if nb_occ == 1 and special_char(toks) == True:
                    ...
                    if word.lower().replace("▁", "") not in closest_words:
                        ...
                        correction = compare_lexique_context(sent, word, closest_words, likely_words, conf_matrix)
...
def main():
    ...
    test[['OCR Text', 'Ground Truth']] = test[['OCR Text', 'Ground Truth']].apply(lambda col: col.str.replace('@', '', regex=False))
    ...
    conf_matrix = pd.read_csv(args.matrix, index_col=0)
    ...
    for gold, sent in tqdm(zip(gold_sentences, ocr_sentences)):
        print(sent)
        corr_sent = correct_sentence(sent, lexique, tokenizer, conf_matrix)
