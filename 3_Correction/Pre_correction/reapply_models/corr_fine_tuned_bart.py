...
def get_results(data: Dataset, preds: list) -> pd.DataFrame:
    ...
    results['old_CER'] = results.apply(lambda row: cer(row['Lexical Correction'], row['Ground Truth']), axis=1)
...
def check_args(file: str, model: str) -> bool:
    ...
    models = [
        'bart-base-ocr-5-epochs-tesseract', 
        'bart-base-ocr-5-epochs-kraken', 
        'bart-base-ocr-10-epochs-tesseract', 
        'bart-base-ocr-10-epochs-kraken', 
        'bart-large-ocr-5-epochs-tesseract', 
        'bart-large-ocr-5-epochs-kraken', 
        'bart-large-ocr-10-epochs-tesseract', 
        'bart-large-ocr-10-epochs-kraken'
    ]
...
def main():
    ...
    parser.add_argument("-t", "--token", type=str, help="Access Token HuggingFace")
    args = parser.parse_args()

    # Vérification des arguments
    valid = check_args(args.file, args.model, args.token)
    if valid == False:
        return    
    ...
    model = AutoModelForSeq2SeqLM.from_pretrained(model_dir, use_auth_token=token)
    ...
    for sample in tqdm(test):
        preds.append(generator(sample['Lexical Correction'])[0]['generated_text'])
